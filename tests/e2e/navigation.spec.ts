...
const pages = [
  ...
  { nav: "任务处理", heading: "任务处理", screenshot: "batch-processing-desktop.png" },
  { nav: "资源归档", heading: "资源归档", screenshot: "archive-desktop.png" },
  { nav: "设置中心", heading: "设置中心", screenshot: "settings-desktop.png" }
];

test("main pages switch without console errors and desktop screenshots are generated", async ({ page }) => {
  const errors: string[] = [];
...
test("settings page starts in simple mode and keeps advanced debug collapsed", async ({ page }) => {
  ...
  await expect(page.getByText("检查依赖")).toHaveCount(0);
  await expect(page.getByText("开发者模式")).toBeHidden();
