...
describe("BdpanCliAdapter", () => {
  it("maps transfer, list, mkdir, rename, move, and share to bdpan commands", async () => {
    const runner = new FakeRunner([
      ...
      { exitCode: 0, stdout: "{\"status\":\"ok\"}", stderr: "" },
      { exitCode: 0, stdout: "{\"status\":\"ok\"}", stderr: "" },
      { exitCode: 0, stdout: "{\"link\":\"https://pan.baidu.com/s/1new\",\"pwd\":\"A7K9\",\"period\":7}", stderr: "" }
    ]);
    ...
    await adapter.createShareLink({ remotePaths: ["panjie/output/task-1"], periodDays: 7 });
    ...
    expect(runner.commands.map((command) => [command.subcommand, ...command.args])).toEqual([
      ...
      ["share", "panjie/output/task-1", "--period", "7", "--json"]
    ]);
  });
  ...
  it("does not fake share success when the share capability is unavailable", async () => {
    ...
    await expect(adapter.createShareLink({ remotePaths: ["panjie/output/task-1"], periodDays: 7 })).resolves.toEqual({
      ok: false,
      error: "分享接口不可用 / 需开通"
    });
  });
