...
- 不采集账号密码，不读取 cookie，不复用浏览器 localStorage 或抓包凭证。
- 遇到失效、违规、风控、验证码、容量不足、权限不足、限流等状态时，只标记失败或进入人工处理。
- 日志、SQLite、导出文件中不得输出账号、密码、cookie、实际授权凭证。
...
| 授权登录 | 仅预留官方 OAuth；真实凭证接入前不保存任何 token |
...
- 同一分享链接重复导入只生成一条主任务。
- 不要求用户输入账号密码。
- 日志、SQLite、导出文件不出现账号、密码、cookie、实际授权凭证。
